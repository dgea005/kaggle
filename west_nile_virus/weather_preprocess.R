##
## script to process weather data
##


# drop some rows with no/little data in them: "Depth", "Water1", "Snowfall"
weather = subset(weather, select = -c(Depth, Water1, SnowFall))

# CodeSum - (from some script)
## process the weather description column into factors, etc
## split up all the descriptions to get unique factors
ee<-levels(weather$CodeSum)
tr<-c()
for (i in 1:length(ee)){
  tr<-c(tr,unlist(strsplit(ee[i],' ')))}
tr<-unique(tr[-1])
tr[8]<-'FGP'  ## rename FG+ to FGP

for (j in 1:length(tr)){
  for (r in 1:nrow(weather)){
    if(grepl(weather[r,'CodeSum'], tr[j])){       ## check if weather descriptor is in obs
